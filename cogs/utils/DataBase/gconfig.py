...
class FilterConfig(object):
    ...
    async def post(self):
        ...
        await self.bot.db.execute(
            query, self.guild_id, self.blacklist_urls, self.whitelist_channels, dumps(self.reasons)
        )
        return self
    ...
    async def update(self) -> None:
        async with self.update_lock:
            query = """UPDATE gconfigs SET blacklist_urls = $1, whitelist_channels = $2, enabled = $3, reasons = $4
                       WHERE guild_id = $5"""
            await self.bot.db.execute(
                query, self.blacklist_urls, self.whitelist_channels, self.enabled, dumps(self.reasons), self.guild_id
            )
