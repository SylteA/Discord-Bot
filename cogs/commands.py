...
class Commands(commands.Cog):
    ...
    async def git(self, ctx):
        """Git download link"""
        embed = discord.Embed(
            title="Git Download Link", description="[Download GIT Here!](https://git-scm.com/downloads)"
        )
        await ctx.message.delete()
        await ctx.send(embed=embed)
    ...
    async def twitter(self, ctx):
        """View Tims Twitter"""
        embed = discord.Embed(
            title="Tech With Tim Twitter!", description="[View Tim's Twitter!](https://twitter.com/TechWithTimm)"
        )
        await ctx.message.delete()
        await ctx.send(embed=embed)
    ...
    async def insta_(self, ctx):
        """View Tims Instagram"""
        embed = discord.Embed(
            title="Tech With Tim Instagram!", description="[View Tim's Instagram!](https://instagram.com/tech_with_tim)"
        )
        await ctx.message.delete()
        await ctx.send(embed=embed)
    ...
    async def pipsearch(
        self, ctx, term, order: lambda string: string.lower() = "relevance", amount: lambda x: min(int(x), 10) = 10
    ):
        ...
        async with ctx.typing():
            ...
            results = int(
                (
                    bs.find("div", class_="split-layout split-layout--table split-layout--wrap-on-tablet")
                    .find("div")
                    .find("p")
                    .find("strong")
                    .text
    ...
    async def suggestion(self, ctx, *, suggestion: str):
        ...
        em.set_author(name=f"Suggestion by {ctx.author.display_name}", icon_url=ctx.author.avatar.url)
    ...
    async def get_docs(self, ctx, key, obj):
        ...
        author = {"latest": "discord.py", "python": "python", "pygame": "pygame"}.get(key, key)
