...
class YouTube(commands.Cog):
        ...
        while not self.bot.is_closed():
            ...
            if self.data is not None:
                ...
                if videos != last_videos:
                    ...
                    print("Finished getting last video, sleeping...")
                    await self.alert_webhook()

            self.data = data["items"][0]
            print("Finished querying youtube stats, sleeping...")
    ...
    async def alert_webhook(self):
        ...
        embed.set_author(
            ...
            url=f'https://www.youtube.com/c/{channelinfo["customUrl"]}',
        )
    ...
    async def stats(self, ctx):
        ...
        statistics = [("Videos", stats["videoCount"]), ("Views", stats["viewCount"])]
        frame = pandas.DataFrame(statistics, columns=("Subscribers", str(stats["subscriberCount"])))
        embed.add_field(
            name="**Statistics**",
            value=f"```{frame.head().to_string(index=False)}```",
            inline=False,
        )
    ...
    async def query(self, ctx, *, title):
        ...
        for index, video in enumerate(videos):
            ...
            if index > 8:
                break
        if not len(string) > 0:
            title = "No videos found D:"
            description = "There were no videos found with your search"
        else:
    ...
    async def subscribe(self, ctx):
        ...
        try:
            ...
            await ctx.send("You will now be notified once tim uploads a new video!")
